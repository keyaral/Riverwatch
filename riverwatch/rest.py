...
def export(request):
    ...
    if request.method == 'GET':
        #images = riverwatch.models.Image.objects.all()
        images = riverwatch.models.Image.objects.order_by('-submission_date').filter(is_approved=True)
        respDict = {}
        respDict["status"] = "OK"
        for image in images:
...
def rest_success(request, img_id):
    ...
    respDict["url"] = reverse('riverwatch.views.image', args=[img_id])
    responseData = json.dumps(respDict)
    response = HttpResponse(responseData, "application/json", 200)
    return response
