from riverwatch.models import Image
...
def to_map_point(image):
    ...
    imageDict["path"] =  '/static/uploaded-images/' + str(image.image_path)
    imageDict["description"] = unicode(image.image_description).encode('ascii', 'ignore')
    imageDict["image_name"] = str(image.image_name)
    imageDict["date"] = str(image.submission_date)
