    ...
    $("#to").val(date.getDate()+"/"+(date.getMonth()+1)+"/"+date.getFullYear());
        ...
        $("#add_tag_filter").click(function(e){
            ...
            if(tag_text != "" && !~tags.indexOf(tag_text)){
                ...
                tags.push(tag_text);
                $("<div class='alert alert-success ' style='padding-top:6px;padding-bottom:6px;margin-bottom:2px'>\
        ...
        $("#add_geo_filter").click(function(){
            ...
            var circleColour = getRandomColour();
            appendFields(geofilters.length+1,circleColour);
            geofilters.push(attachGeoFilter(university, geofilters.length+1,circleColour));
...
filterJavascript(dateChecked, tagsChecked, geoChecked, fromDate, toDate, tags,geoFilters);
});
});
...
function attachGeoFilter(position, id, color){
...
function appendFields(field_id, color){
    var container_div = $("<div id='set_"+field_id+"' class='well'>");
...
var delete_button = $("<h4>Filter "+field_id+"  <span><button id='delete_"+field_id+"' class='delete btn btn-danger btn-xs ' style='background-color: "+color+"'>Delete</button></span></h4>");
...
function resetFilters(){
    for (var i = regions.length - 1; i >= 0; i--) {
        for (var j = regions[i].markers.length - 1; j >= 0; j--) {
            regions[i].markers[j].filtered = false;
        }
...
function filterJavascript(dateChecked, tagsChecked, geoChecked, fromDate, toDate, tagsArray, geoFilters){
    resetFilters();
    regionDropdown("All");
    ...
    if(dateChecked){
        ...
        fDate = new Date();
        fDate.setFullYear(fromDate[2],parseInt(fromDate[1])-1,fromDate[0]);
        tDate = new Date();
        toDate = toDate.split("/");
        tDate.setFullYear(toDate[2],parseInt(toDate[1])-1,toDate[0]);
        ...
        for (var i = regions.length - 1; i >= 0; i--) {
            ...
            for (var j = regions[i].markers.length - 1; j >= 0; j--) {
                ...
                if (regions[i].markers[j].date >= fDate && regions[i].markers[j].date <= tDate) {
                    regions[i].markers[j].filtered = false;
                }
                else{
                    regions[i].markers[j].marker.setVisible(false);
                    regions[i].markers[j].filtered = true;
                    numFiltered+=1;
            ...
            regions[i].text.setMap(null);
            var mapLabel = {
             text: regions[i].markers.length-numFiltered,
             position: regions[i].center,
             map: map,
             fontColor: 'white',
             strokeColor: regionStrokeCol,
 ...
 if(tagsChecked){
    ...
    for (var i = regions.length - 1; i >= 0; i--) {
            ...
            for (var j = regions[i].markers.length - 1; j >= 0; j--) {
                ...
                for (var k = tagsArray.length - 1; k >= 0; k--) {

                    if( (regions[i].markers[j].tags.indexOf(tagsArray[k])) == -1){
                        regions[i].markers[j].filtered = true;
                ...
                if(regions[i].markers[j].filtered == true){
                    regions[i].markers[j].marker.setVisible(false);
                    numFiltered+=1;
                };
            };
 ...
 if(geoChecked){
    ...
    for (var i = geoFilters.length - 1; i >= 0; i--) {
        bounds.push(geoFilters[i].circle.getBounds());
    }
    ...
    for (var i = regions.length - 1; i >= 0; i--) {
        ...
        for (var j = regions[i].markers.length - 1; j >= 0; j--) {
                ...
                if(bounds[k].contains(regions[i].markers[j].marker.getPosition())){
                    if(regions[i].markers[j].filtered != true){
                    regions[i].markers[j].filtered = false;
...
if(!dateChecked && !tagsChecked && !geoChecked){
    DeselectRegions();
    resetFilters();
